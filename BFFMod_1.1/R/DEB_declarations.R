...
.Deb_cohort$parms <- c(  #passed to Fortran surbroutines, KEEP EXACT SAME NAMES & ORDER
    ...
    # DEB PARAMETERS #
    #----------------#
    aeff   = 0.64,    # assimilation efficiency,ESTIMATED      [-] (Sara 2011 0.88)
    # (directly from DEB pars)
    kappa  = 0.5,     # fraction energy to structure  [-] Sara etal 2012 
    # (directly from DEB pars)
    
    maint  = 0.0152,   # carbon-specific maintenance rate [(mmolC/mmolC)/d] -> [/d]
    # DEB.Pm     # == [Pm]   -> Volume specific Maintenance cost (rate) for structure (J/cm3/d)
    #  basal respiration rate ==> [pM]/(J/cm3) => [J/cm3/day] To [/day]
    # maint = [24] (Sara et al) / J_cm3 [1575] = 0.0152381
    
    cat_s  = 0.2671488,   # surface-specific catabolism rate [mmolC/cm2/d] 
    # surface-specific catabolism  ==> {p.Am} / [Em] *(mmol.C/cm3) # [mmol.C/cm2/d]
    # => VolToMol "energy conductance"
    # cat = (DEB.P.Amax /DEB.E.max) * [3] (mmolC_cm3) ==> 
    # P.Amax must be calculated based on pure phytoplankton feeding under optimal circumastances
    # max_phyto_filt = 1.038794, #[mmolC/cm2/d] (filtered phyto max)
    # MPI =rho_Phyto * max_phyto_filt / (1.d0 + ((rho_phyto*FR_Phyto(I))/max_Phyto_ing)) #max phyto ing
    # P.A.max ~ aeff * (ncPhyto/ncMussel) * MPI =  0.4842135 [mmolC/cm2/d] ~ 254 [j/cm2/d]
    # DEB [Em] = 2190 j/cm3 (sara et al 2012 & vdVeer et al., 2006)
    # {p.Am} / [Em]*mmolC_cm3 -> 254/2190*3 ~ 0.3479452
    
    # equal to literature v values !!!!
    ...
    # 1) Wet Weight including shell (live weight/Total weight), here: TW 
    # 2) Wet weight excluding shell (wet meat weight) here: WW (0.2191 used)
    # 3) Dry weight including shell (dry weight) here: not used 
    # 4) Dry weight excluding shell (dry meat weigth/ often just DW) here: DW
    ...
    # --> gC_gAFDW = 0.5! (0.4/0.8 = 0.5) -> same amount of C
    # ~0.79 is based on Palmerini & Bianchi 1994 (A), Saraiva et al. estimated 0.85
    # so this fits literature nicely. 
    ...
    # gAFDW/molC = 24 (23.9 # often used conversion for pure organics (0.5020921 %C))
    
    # gdw/molC = 30  #!! 40% Carbon value (0.09/3*1000) (used here.)
    # gDW/molC  would be 26.66667 at 45% carbon value of dry weight (sometimes used)
    ...
    # Casas & Bacher calculated 1gWW/cm3 also. 
    # I do not agree with this assumption. 
    
    # 0.09 gDW_cm3 is often used (DEB default) %C  
    # (monaco & mcquaid M.gallo/ Wijsman M.edulis, AmP)
    # dry weight J coulper used: 5.71e-5 [g/joule]
    ...
    gWW_gDW = 5.7, # ESTIMATED ( 7 in brigolin, 11 in saraiva, 4.8 in Palerini)
     
    
    
    #gWW_gDW = 2.62, # gram WW (no shell) per gram DW (no shell with ash) (1/0.3818281)
    #gWW_gDW = 3.022069, # calcultaed from other ratios
    #  gTW_gAFDW   = 17.24138, # from gAFDW to gram total WW (including shell!) 
    # (Palmerini & Bianchi 1994) ~ drained from external water (blotted) including shell
    # average of 2 methods = 0.065 TW:AFDW -> 15.4 gWW/gAFDW
    # (Brigolin 2009) : from gram DW to gram WW  = 7 / gDW to TW  17 

    # gTW_gWW = 4.564126 # SINOP LINEAR MODEL FIT!
    # gTW_gAFDW = 25

    
    del.M     = 0.23,# from equivalent spherical length to length [-] (Sara, 2011)
    ...
    e_dens_trh    = 0.75, # avg energy density at which pop mortality starts [mmolC R / mmolC S] CALIBRATED
    mort_starv = 0.027, # starvation-induced mortality rate [/d] (0.009) CALIBRATED
    mort_base   = 0.00041, # min population mortality due to environment [/day] (Perez camacho 1991)& Brigolin 2006
    # = 0.15/365 (often used mussel mortality) [NOW CALIBRATED TO COMPENSATE FOR OTHER MORTS]
    ...
    spawn_peak_2 = 272, # [julian day] day at second spawning occurs (~oct 15) # ESTIM
    ...
    v_settle        = 588,         # [m/day], base settling rate of settling larvae
   ...
   T_l  = 273.15+4, # lower boundary of tolerance range [[K]] (was 275) 273.15+(4-10) (Jansen 2006/2009) ABT
   T_h  = 273.15+27, # upper boundary of tolerance range [[K]] (was 296) 273.15+(27-30) (Jansen 2009) ABT
   ...
   # Lazo & Pita 13.7% larva survival after 19 days ~ > log(0.137)/19 = 0.105
   mort_larva    = 0.14,  # [/day] per capita density dependent morality rate larvae (here estim)
   growth_larva  = 0.13 ,  # [/day] max daily rate of increase (80 ng to 630 ng (widdows in 16 days (lazo&pita)))
   ks_larva      = 10,      # [mmol C/m3], mich-ment half concentration of phytoplankton on larvae
   max_set_rate = 1000, # max daily settlement rate of larva [ind/m2/d] toupoint etal 2012 /m/d ~ 100-200
   #ks_settlement = 2000, # competent pediveliger density at which half of max set rate is reached [ind/m3]
   #l_sink =  588, # [m/s] sinking speed mussel pediveligers
   tc = 0.08, # [N/m2] critical shear stress for settling pediveligers
   ...
   # 15k cells /ml * 1picomolC/cell -> 10000*1e-12= 1e-8 molC/ml
   # 1e-8molC/ml = 0.01molC/m3 = 15 ~ 10 mmolC/m3 at half max growth.
   
   # ENVIRONMENT OF THE LOCAL WATERBOX (only for cohort model)
   
   water_renew = 3,    # water renewal rate (based on hydrology)  [m3/day] (simplified version = 1/depth, assuming only sinking of 1m/d?)
   O2_renew    = 3,    # O2 renewal rate  [/day] equal to water renew, can set high to diminish O2 effect
   depth  = 5,         # sediment depth [m] relevant for box volume, this is not equal across black sea bottom forcing boxes
   
   max_clear = 0.2,  # [m3/d/cm2] (Saraiva 2011 M.edul, 0.096)  [[double max measured]]  
   max_phyto_filt = 0.87, #[mmolC/cm2/d] (Rosland 2009 M.edul = 0.48) ESTIMATED (1.3)
   max_DT_filt = 2.40,   #[mmolC/cm2/d] # ESTIMATED (1.78)
   max_spm_filt = 3.5,   #[g/cm2/d] (Saraiva 2011 M.edul)
   
   ### INGESTION/PSEUDOFAECES PARAMETERS
   rho_Phyto = 0.99,   #[-] (Saraiva 2011 M.edul)
   rho_DT = 0.73,      #[-] # ESTIMATED (prev 0.95/0.86)
   rho_SIM = 0.45,    #[-] (Saraiva 2011 M.edul)
   max.ing.Phy = 10.8, # [mmolC/d] (Saraiva 2011 M.edul) (prev = 9.2)  ESTIMATED
   max.ing.DT = 19.7, # [mmolC/d] max DT carbon uptake # ESTIMATED (prev = 20) 
   max_SIM_ing = 0.23, # [g/d] (Saraiva 2011 M.edul = 0,23) 
   ...
   nc_DT = 0.04,   # ~ 15 CN ~ Banaru et al., 2007 (black sea sediment pom nc ratio) 0.067 / 0.05 / 0.02 / 0.04 
   ...
   mort_juv = 0.03, # mort at smallest mussel size, inital thinning (Perez camacho 1991) (0.03?)
...
.Deb_cohort$forc <- data.frame(names =  #names from Fortran surbroutines, KEEP EXACT SAME NAMES & ORDER
   c("f_Phyto", "f_Temp", "f_Detritus", "f_Sim", "f_O2", "f_tau"))
...
.Deb_cohort$forc$description <- c(
   ...
   "External oxygen concentration", "bottom stress")
.Deb_cohort$forc$units <- c(
   "mmolC/m3", "dgC", "mmolC/m3", "g/m3", "mmolO2/m3", "N/m2")

# instate auxiliary variables/quota used for utilities, not passed to FORTRAN code
.Deb_cohort$auxiliary <- c(
  ...
  # based on linear fit Flesh weight/Total weight Karayucel et al., 2010
  POM_g_to_mmolC = 41.67, # POM (g) -> POC (mmolC) ratio used in calibration procedures
  CHL_to_PHY = 3.95768, # chlorophyll a (ug) -> phytoplankton (mmolC) BAMHBI  conversion at Redflied ratio
  PHYTO_g_to_mmolC = 44.17543, # phytoplankton weight (g) -> biomass (mmolC) ratio
  DETRITUS_g_to_mmolC = 38.21526 # mmolC per gram DETRITUS (0.04 N:C)
)
