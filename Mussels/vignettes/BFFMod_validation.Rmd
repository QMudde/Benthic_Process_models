---
title: "BFFMod: Validation of Mytilus individual growth with DEB based model"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{BFFMod Validation of DEB mechanism}
...
## Validation example of the *BFFMod* package.

Validation of individual growth DEB model based on Black Sea growth curves.\ 
Model parameters are fitted on growth curves of *Mytilus galloprovincialis* growth data 
originating from Black Sea & Marmara Sea.  
Besides length growth, data on biomass, mortality & physioliological rates are used to 
calibrate the default model parameters.

#### References:
Galimany et al., 2015 (physiological rates data)   
Karayusel et al., 2010 (Sinop data)  
Vural et al., 2015 (Marmara Sea data)   
Chelyadina et al., 2020 (Crimea data)   

### Initialisations of objects from the BFFMod package
```{r}
...
Before using the data in the BFFMod model functions, we need to set the values in the right format.   
The *get_INI_indiv* function is used to set initial values for the model.   
The *make_ff* function is used to construct forcing function in the correct format, 
with recalculated units.
```{r}
## Sinop data 
...
#### setting sensitivity to parameter values 
Here, we determine the parameters that we should iterate over, and the magnitude of the 
changes of that parameter, these relative values will be the same in the whole document.
```{r}
...
```
We set a sensitivity range of 10% and chose DEB parameters & feeding parameters to vary in the iterations

### sensitivity at Sinop (Turkey)
Both the Length and biomass data were used for calibration of the model   
...
### sensitivity at Marmara Sea
Both the Length and biomass data were used for calibration of the model   
*Growth and survival performances of Mediterranean mussel (Mytilus  galloprovincialis, Lamarck, 1819) on different depths in Cardak lagoon,  Dardanelles.*  
Vural et al., 2015
...
### Validation & Sensitivity of growth curves at Sevastopol bay (Crimea)
These data are only used for validation of the individual growth model   
...
### Physiological rates of *Mytilus galloprovincialis* at Alfacs Bay (Spain)
...
### plots of physiological rates, data are used for inital calibration
```{r, fig.width=6, fig.height=6}
...
### Optional addition (this is where the model diverges most from the data)
```{r, fig.width=6, fig.height=6}
