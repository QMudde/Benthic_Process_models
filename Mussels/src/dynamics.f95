...
!-------------------------------------------------------------------------------
  ...
  Ingestion(I) = Phyto_Ing+DT_Ing
  Ingestion_N = Phyto_Ing*nc_Phyto + DT_Ing*nc_DT
  ...
  ! Here, we include addional assimilation penaly for the composition of the ingested food
  ! Current formulation assumes FoodNC cannot exceed nc_tissue
  ! ingested SIM only inhibits filtration and ingestion but not

  Assimilation(I) = (Ingestion(I) * AE ) 
  Assimilation_N = Assimilation(I) * nc_tissue
  
  ! Everything left is excreted
  Faeces_production(I) = Ingestion(I) - Assimilation(I) ! fecal pallets
...
DOUBLE PRECISION :: total_mort, overcrowd, crowdingvar, crowd_mortality, &
                    size_mortality, partial_starvation, env_mortality, RSratio2
...
!   mort_env = mort_base + (  (mort_juv-mort_base) * (1.d0 - Length(I) / (Length(I)+recr_length))   )
   ...
   ! rs ratio below 1 <- partial starvation starts
   ...
   RSratio2      = RESERVE(I)/(STRUCT(I)+1d-8) ! this is not a global term so calculate again here
      ...
      IF (RSratio2 < e_dens_trh) THEN 
         ...
         partial_starvation = mort_starv * (e_dens_trh - RSratio2)
   ...
   ! assume reference mort_env is for reference temperature (T_factor = 1)
   ...
   env_mortality = mort_base*(1.d0 + (1.d0 - T_factor)) ! base + increase  in cold & heat
   ELSE
   env_mortality = mort_base
   END iF
   
   ! shell size based mortality
   ...
   total_mort = env_mortality + crowd_mortality + partial_starvation + size_mortality
      ...
      Mortality(I)  = total_mort*POPULATION(I)
...
! this is then increased by the settle speed, which can be changed in parms

  Set_rate = max_set_rate * comp_pedi / (comp_pedi + ks_settlement_field) 
...
! Larva mortality : most important factor for larvae controll
    ! biotic factors most important for larva mortality (Thorson, 1950)
    ! assumption: temperature can be a proxy for predation pressure?
...
!--------------------------------------------
! Dynamics of Larva Density & Biomass Pools
!---------------------------------------------

  dLARV_DENS =    Total_spawn/weight_egg/depth/2.d0*(1.d0-mort_egg)   & !half female ratio
...
! recruits are proportion settlers that survive the early settlement mortality   
    ...
    Settle_rate(new) = Set_rate                  ! output variable [indivs/day]
...
! previous: dL_Biom = recruits*LARV_BIOM
 ...
 ! I HAVE DB AT THE OTHER SIDE ? ! we have: dW/dt = (dB/dt-W dN/dt - dB/dt-)/N   ?
...
!-------------------------------------------------------------------------------
    ...
    !dPHYTO    = water_renew*(f_PHYTO    - PHYTO)                    &
    !           - Total_Ingestion * p_phyto/depth
    
    dPHYTO     = water_renew*(f_PHYTO - PHYTO)                       &
                - Total_FR_Phyto/depth
               
    !dDETRITUS = water_renew*(f_DETRITUS - DETRITUS)                 &
    !           - Total_Ingestion * (1.d0-p_phyto)/depth
               
    dDETRITUS = water_renew*(f_DETRITUS - DETRITUS)                  &
               - Total_FR_DETRITUS/depth
               
    dSIM      = water_renew*(f_SIM - SIM)
    dO2       = water_renew*(f_O2 - O2) - 1.070312 * Total_respiration/depth
